...
void run(const char* filename) {
    ...
    std::chrono::milliseconds duration = std::chrono::duration_cast<std::chrono::milliseconds>(t);

    cout << "Funnel tree initialized with " << size(tree) << " nodes in " << duration.count() << " miliseconds.\n";
    cout << "There are " << tree.size() << " levels of the FunnelTree\n\n";

    return;
}

int main(int argc, char *argv[]) {
    freopen("out_parallel.txt", "w", stdout);
    run(argv[1]);
    fflush(stdout);
    fclose(stdout);
}
